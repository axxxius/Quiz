...
export const TeamForm = ({ handleClick }: TeamFormProps) => {
    const { register, handleSubmit, formState: { errors } } = useForm<TeamFormValues>({ mode: 'onSubmit' });
    ...
    return (
        <form className={styles.form} onSubmit={handleSubmit(onSubmit)} >
            ...
            {errors.name &&
                <Typography variant='text_16_r'>
                    {errors.name.message}
                </Typography>
            }
            <div className={styles.button_container}>
                <Button className={styles.button} type="submit">Создать команду</Button>
            </div>
        </form>
    )
}